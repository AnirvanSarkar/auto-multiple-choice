#
# Copyright (C) 2008-2026 Alexis Bienvenüe <paamc@passoire.fr>
#
...
sub calage {
    ...
    if ( $self->{type} =~ /^h/i ) {
        ...
        if ( abs( cos($theta) ) > abs( sin($theta) ) ) {
            $alpha = ( crochet( $cx, $cxp ) + crochet( $cy, $cyp ) ) /
              ( $den * cos($theta) );
        } else {
            $alpha = ( crochet( $cx, $cyp ) - crochet( $cxp, $cy ) ) /
              ( $den * sin($theta) );
        }
        ...
        $self->{t_e} =
          moyenne(@$cxp) -
          $alpha *
          ( moyenne(@$cx) * cos($theta) - moyenne(@$cy) * sin($theta) );
        $self->{t_f} =
          moyenne(@$cyp) -
          $alpha *
          ( moyenne(@$cx) * sin($theta) + moyenne(@$cy) * cos($theta) );
        ...
        $self->{t_e} = moyenne(@$cxp) -
          ( $self->{t_a} * moyenne(@$cx) + $self->{t_b} * moyenne(@$cy) );

        ( $self->{t_c}, $self->{t_d} ) =
          resoud_22( $sxx, $sxy, $sxy, $syy, $sxyp, $syyp );
        $self->{t_f} = moyenne(@$cyp) -
          ( $self->{t_c} * moyenne(@$cx) + $self->{t_d} * moyenne(@$cy) );
...
sub xml {
    ...
    my $pre = " " x $i;
    my $r   = $pre
      . sprintf( "<transformation type=\"%s\" mse=\"%f\">\n",
        $self->{type}, $self->{MSE} );
    $r .= $pre . "  <parametres>\n";
    if ( $self->{type} =~ /^[hl]/i ) {
        $r .= $pre . "      <a>" . $self->{t_a} . "</a>\n";
